...
router.post("/profiles/:profileId/progress/entry-feedback", async (req, res) => {
  ...
  try {
    ...
    {
      const cap = await enforceAiCap(req.user.id, "progress-feedback");
    ...
    const result = await generateEntryFeedback(
      ...
      guidebookMarkdown,
      gpaEntries,
      resolveAuthedLocale(owner.profile, req),
    );
...
router.post("/profiles/:profileId/progress/analyze", async (req, res) => {
  ...
  try {
    ...
    {
      const cap = await enforceAiCap(req.user.id, "progress-analyze");
      if (!cap.allowed) { res.status(cap.status).json({ error: cap.error }); return; }
    }
    ...
    const result = await generateProgressAnalysis(
      ...
      resolveAuthedLocale(profile, req),
    );
